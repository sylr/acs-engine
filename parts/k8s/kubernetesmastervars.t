...
{{if not IsHostedMaster}}
  ...
  {{else}}
    ...
    "virtualNetworkName": "[concat(parameters('clusterName'), '-vnet-', variables('nameSuffix'))]",
...
{{if not IsHostedMaster }}
    {{if IsPrivateCluster}}
        ...
        {{end}}
    {{else}}
        "masterPublicIPAddressName": "[concat(parameters('clusterName'), '-master-ip-', variables('masterFqdnPrefix'), '-', variables('nameSuffix'))]",
        "masterLbID": "[resourceId('Microsoft.Network/loadBalancers',variables('masterLbName'))]",
        "masterLbIPConfigID": "[concat(variables('masterLbID'),'/frontendIPConfigurations/', variables('masterLbIPConfigName'))]",
        "masterLbIPConfigName": "[concat(parameters('clusterName'), '-master-lbFrontEnd-', variables('nameSuffix'))]",
        "masterLbName": "[concat(parameters('clusterName'), '-master-lb-', variables('nameSuffix'))]",
        "kubeconfigServer": "[concat('https://', variables('masterFqdnPrefix'), '.', variables('location'), '.', variables('fqdnEndpointSuffix'))]",
    {{end}}
{{if gt .MasterProfile.Count 1}}
    "masterInternalLbName": "[concat(parameters('clusterName'), '-master-internal-lb-', variables('nameSuffix'))]",
    "masterInternalLbID": "[resourceId('Microsoft.Network/loadBalancers',variables('masterInternalLbName'))]",
    "masterInternalLbIPConfigName": "[concat(parameters('clusterName'), '-master-internal-lbFrontEnd-', variables('nameSuffix'))]",
...
{{end}}
    "masterLbBackendPoolName": "[concat(parameters('clusterName'), '-master-pool-', variables('nameSuffix'))]",
    "masterFirstAddrComment": "these MasterFirstAddrComment are used to place multiple masters consecutively in the address space",
    "masterFirstAddrOctets": "[split(parameters('firstConsecutiveStaticIP'),'.')]",
    "masterFirstAddrOctet4": "[variables('masterFirstAddrOctets')[3]]",
    "masterFirstAddrPrefix": "[concat(variables('masterFirstAddrOctets')[0],'.',variables('masterFirstAddrOctets')[1],'.',variables('masterFirstAddrOctets')[2],'.')]",
    "masterVMNamePrefix": "[concat(parameter('clusterName'), '-master-', variables('nameSuffix'), '-')]",
    "masterVMNames": [
...
{{else}}
    ...
    "agentNamePrefix": "[concat(parameters('clusterName'), '-agentpool-', variables('nameSuffix'), '-')]",
{{end}}
